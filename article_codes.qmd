...
We have used the Corine Land Cover 2018 (Version 2020_20u1), downloaded from [Copernicus](https://land.copernicus.eu/pan-european/corine-land-cover). The resolution of the raster is 100 m x 100 m. We classify as potential wildland vegetation the following classes @fig-corine-fuel-map-norway:
...
We use the fuel map produced by [@Aragoneses2022]. The original resolution of the map is 1km, so we resampled it to a grid cell of 250x250 metres (same grids than `region_r`) based in the type that is most presented in the area; i.e., `mode` (@fig-firEUrisk-fuel-map-norway).
...
Area of wild vegetation by county (@tbl-summary-table-areas): we calculate the number of vegetation cells (i.e., grid centroid is inside the polygon), and calculate the area based on the raster resolution.
...
```{r}
#| label: tbl-buildings-wui
#| tbl-cap: Number of buildings in each WUI classification 
...
We have analysed all wildfires that have affected buildings from 2016 to 2023 (i.e., `r dim(wf)[1]`; `r dim(filter(wf, country == "NO"))[1]` in Norway and `r dim(filter(wf, country == "SE"))[1]` in Sweden). The can be obtained from the *Norwegian Directorate for Civil Protection (DSB)*  (Norway) and the *Swedish Civil Contingencies Agency (MSB)* (Sweden). [Note: They were not uploaded in our GitHub repository]

## Wilfires and building density

Calculate the building density of the grid of 250 x 250 m where the wilfire originated.

```{r}
#| label: fig-wilfires-number-buildings
#| fig-cap: Number of buildings per grid cells of 250 m x 250 m and wildfires (red points)
...
```{r}
#| label: fig-wildfires-buildings-density
#| fig-cap: Histogram of building density in each wildfire
...
```{r}
#| label: tbl-wildfires-buildings-density
#| tbl-cap: Number of wildfires
