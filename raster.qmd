...
#' @param .cache_path File path to save the results 
...
#' @param .max_distance Maximun distance in m (e.g., 2400 m)
...
add_colours_wui <- function(.x) {
  ...
  levels(r) <- df_lev_lst
  # Add colour table 
  df_col <- data.frame(id = 0:2, col = c("#E7EEE7", "#FF6633", "#003366"))
...
p1 <- ggplot() +
  ...
  coord_sf(xlim = c(2500000, 5402285),
           ...
           expand = FALSE) +
  theme_bw() +
  theme(panel.background = element_rect(fill = "#E7EEE7")) +
...
plot(region_r, col = "#E7EEE7", axes = FALSE, legend = FALSE)
plot(effis_fuel_cls, 
...
plot(region_r, col = "#E7EEE7", axes = F, legend = F)
...
plot(no_v_p, col = "#E7EEE7", axes = FALSE, legend = FALSE)
...
The original resolution of the map is 10 meters. We aggregate it (by a factor of 10) and transform the coordinate system from `SWEREF99 TM (EPSG:3006)` to `ETRS89-extended / LAEA Europe (EPSG:3035)`.
...
Finally, we combine both maps in one (@fig-wild-vegetation-national)
...
Original data from [@Aragoneses2022]. Al fuels are considered for WUI. The resolution of the original map is 1km and we have re-sampled it to a grid cell of 250x250 grids (same than `region_r`) for the WUI (based in the type that is most presented in the area: `mode`).
...
We carry out a point-based approach with grid cells of `r raster_resolution` m x `r raster_resolution` m, and a buffer radio of `r radio_buffer` m. Our workflow is therefore:
...
flowchart TD
  ...
  B --  No   --> D[No WUI]
  C --  Yes  --> E[Intermix] 
  C --  No   --> F{within 2.4 km of large forests (>5 km2)}
  F --  Yes  --> G[Interface]
  F --  No   --> H[No WUI]
  style E fill:#CC79A7
  style G fill:#F0E442
...
Zoom to counties (@fig-wui-map-counties; @tbl-summary-wui-county):

```{r}
#| label: fig-wui-map-counties
#| fig-cap: "WUI maps by county based on different fuel/vegetation maps (resolution of 250 x 250 m)"
#| fig-subcap: 
#|  - "Viken"
#|  - "Rogaland"
#|  - "Vestland"
...
#   coltab(wui_r_zoom) <- 
#     data.frame(id = 0:2, 
#                col = c("#E7EEE7", "#F0E442", "#CC79A7"))
#   
#   par(mfrow = c(1, 4))
#   
#   plot(wui_r_zoom$effis, 
#        axes = FALSE, 
#        font.main = 1,
#        legend = FALSE,
#        mar = c(0, 0, 0, 0),
#        main =  names(wui_r_zoom$effis))
#   # title( {{ .x }}, line = -4, adj = 0)
#   
#   plot(wui_r_zoom$corine, 
#        axes = FALSE, 
#        font.main = 1,
#        legend = FALSE,
#        mar = c(0, 0, 0, 0),
#        main =  names(wui_r_zoom$corine))
#   
#   plot(wui_r_zoom$firEUrisk,
#        axes = FALSE, 
#        font.main = 1,
#        legend = FALSE,
#        mar = c(0, 0, 0, 0), 
#        main =  names(wui_r_zoom$firEUrisk))
#   
#   plot(wui_r_zoom$sr16,
#        axes = F, 
#        font.main = 1,
#        legend = FALSE,
#        mar = c(0, 0, 0, 0), 
#        main =  names(wui_r_zoom$sr16)) 
#   
# }
# 
# # par(mfrow = c(1, 11))
# as.list(counties$NUTS_NAME) |> 
#   walk(plot_zoom)
...
tbl_summary_county <- function(.x) {
  .x |> 
    ...
    tab_options(
      table.font.size = 9,
      row_group.background.color = "#E7EEE7",
      row_group.padding = px(1),
      row_group.font.weight = "bold",
      data_row.padding = px(1)
    ) 
...
Area of wild vegetation in each county: Number vegetation cells (centroid is inside the polygon), sum then and calculate the area based on the raster resolution

```{r}
#| label: tbl-summary-table-areas
#| tbl-cap: Number of buildings and wildland vegetation percentage by county 
#| cache: true


# Number of buildings (use building_count?) ----
...
# Percentage vegetation county ----
# EFFIS
effis_county <- effis_wild_veg |> 
  # Number of cells
  extract(y = vect(counties),
          fun = "sum",
          na.rm = TRUE,
  ...
  mutate(
    area_km = wild_veg * ((res(effis_wild_veg))[1]/1000 * (res(effis_wild_veg ))[2]/1000)
    ) |> 
  # Get only values 
  st_drop_geometry() |> 
...
corine_county <- corine_wild_veg |> 
  ...
  # Area (number cells x area cells)
  mutate(
    area_km = wild_veg * ((res(corine_wild_veg))[1]/1000 * (res(corine_wild_veg ))[2]/1000)
    ) |> 
  # Get only values 
  st_drop_geometry() |> 
  select(NUTS_NAME, CNTR_CODE, area_km) |> 
...
sr16_county <- sr16_wild_veg |> 
  ...
  # Area (number cells x area cells)
  mutate(
    area_km = wild_veg * ((res(sr16_wild_veg))[1]/1000 * (res(sr16_wild_veg ))[2]/1000)
    ) |> 
  # Get only values 
  st_drop_geometry() |> 
  select(NUTS_NAME, CNTR_CODE, area_km) |> 
...
nmd_county <- nmd_wild_veg |> 
  ...
  # Area (number cells x area cells)
  mutate(
    area_km = wild_veg * ((res(nmd_wild_veg))[1]/1000 * (res(nmd_wild_veg ))[2]/1000)
         ) |> 
  # Get only values 
  st_drop_geometry() |> 
  select(NUTS_NAME, CNTR_CODE, area_km) |> 
...
# FirEUrisk
firEUrisk_county <- firEUrisk_wild_veg |> 
  # Number of cells
  extract(y = vect(counties),
          fun = "sum",
          na.rm = TRUE,
  ...
  # Area (number cells x area cells)
  mutate(
    area_km = wild_veg * ((res(firEUrisk_wild_veg))[1]/1000 * (res(firEUrisk_wild_veg ))[2]/1000)
         ) |> 
  # Get only values 
  st_drop_geometry() |> 
  select(NUTS_NAME, CNTR_CODE, area_km) |> 
...
# Joint tables
county_veg <- effis_county |> 
  left_join(corine_county) |> 
  left_join(national_county) |> 
...
Equivalent to select any area with building as WUI.
...
p_1 <- ggplot() + 
  geom_spatraster(data = wui_r, maxcell = 2000000) +
...
df_col <- data.frame(id = 0:2, col = c("#000000", "#E69F00", "#009E73"))
df_col_lst <- replicate(nlyr(wui_r), df_col, simplify = FALSE)
coltab(wui_r) <- df_col_lst
plot(wui_r) 
...
We analyse all wildfires (i.e., 330) that have affected buildings from 2016 to 2022. The data have been obtained from XX (Norway) and XXX (Sweden). 
...
wf_se <- readxl::read_xlsx("data/wui_fires_sweden_epsg3006.xlsx") |> 
  ...
  filter(datetime >= lubridate::as_date("2016-01-01")) |> 
...
ggplot() +
  geom_sf(data = region_sf, fill = "#E7EEE7") +
  geom_sf(data = country_sf, fill = NA) +
...
Intersect points with WUI maps

## Effect distance to large forest

Get if the grid to which the wildfire is located was defined as WUI in the respective maps.
